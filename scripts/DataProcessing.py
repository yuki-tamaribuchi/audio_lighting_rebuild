...
class DataProcessing():
    ...
    def create_color_data(self):
        ...
        def convert_rgb_to_xy(data):
            r_gamma = pow( ((data[0]/255) + 0.055) / (1.0 + 0.055), 2.4 ) if (data[0]/255) > 0.04045 else ((data[0]/255) / 12.92)
            g_gamma = pow( ((data[1]/255) + 0.055) / (1.0 + 0.055), 2.4 ) if (data[1]/255) > 0.04045 else ((data[1]/255) / 12.92)
            b_gamma = pow( ((data[2]/255) + 0.055) / (1.0 + 0.055), 2.4 ) if (data[2]/255) > 0.04045 else ((data[2]/255) / 12.92)
